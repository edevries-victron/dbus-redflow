...
QString VBusItemPrivateCons::qGetDescription(const QString &language, int length)
{
	if (mDescriptionObtained)
		return mDescription;

	QDBusPendingReply<QString> descr = mProxy->GetDescription(language, length);
	QDBusPendingCallWatcher *watcher = new QDBusPendingCallWatcher(descr, this);
	this->connect(watcher, SIGNAL(finished(QDBusPendingCallWatcher*)), SLOT(descriptionObtained(QDBusPendingCallWatcher*)));
...
int VBusItemPrivateCons::qSetDescription(const QString &language, const QString &description)
{
	Q_UNUSED(language);
